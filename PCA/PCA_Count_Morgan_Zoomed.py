...
folder_path = r'C:\Users\Ellie\Documents\Chemistry\Year 3\Miniproject\Working Code for PCA'
file_name = 'countmorgan_pca.csv'
file_path = os.path.join(folder_path, file_name)

Data_pca = pd.read_csv(file_path)  # Load the CSV file into a pandas DataFrame
...
filtered_data_test = test_data[(test_data['PC1'] >= -20) & (test_data['PC1'] <= 10) &
                         (test_data['PC2'] >= -10) & (test_data['PC2'] <= 10)]

filtered_data_train = train_data[(train_data['PC1'] >= -20) & (train_data['PC1'] <= 10) &
                         (train_data['PC2'] >= -10) & (train_data['PC2'] <= 10)]

train_data = train_data.iloc[:, [0, 1]]  


# Combine data for axis scaling
combined_data = np.vstack((filtered_data_train, filtered_data_test))
...
scatter_train = ax.scatter(filtered_data_train.iloc[:, 0], filtered_data_train.iloc[:, 1], color='black', label="Training Set", alpha=0.7)
scatter_test = ax.scatter(filtered_data_test.iloc[:, 0], filtered_data_test.iloc[:, 1], color='red', label="Testing Set", alpha=0.7)
...
plt.title("Count Morgan PCA Plot: Training vs Testing Sets Zoomed")
plt.legend(loc="upper right")
plt.grid(True)
plt.axis("equal")
plt.savefig('count_morgan_pca_zoomed.png', dpi=300, bbox_inches='tight')
